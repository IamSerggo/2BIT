...
*/
	...
	if (q == NULL) queueError(QERR_INIT); //osetrenie podmienky
	else{
		for (int i = 0; i < QUEUE_SIZE; i++) q->arr[i] = '*'; //naplnim radu znakom *
		q->f_index = 0;
		q->b_index = 0; //pociatocne indexy na 0
	}
...
*/
	...
	return (++index % QUEUE_SIZE); //posuniem sa o jedno, ked pridem na koniec posuniem sa na zaciatok
	  //solved = FALSE;                  /* V p��pad� �e�en�, sma�te tento ��dek! */
...
*/
	...
	return((q->f_index == q->b_index) ? 1 : 0); //ak sa prvy index rovna prvemu
																							//volnemu fronta je prazdna
...
*/
	...
	return(nextIndex(q->b_index) == q->f_index ? 1 : 0); //pokial sa posuniem na poslednom o jeden a vratim sa na zaciatok
																											 //fronta je plna
	  //solved = FALSE;                  /* V p��pad� �e�en�, sma�te tento ��dek! */
}
...
*/
	...
	if(queueEmpty(q) == 1) queueError(QERR_FRONT); //osetrenie podmienky
	else *c = q->arr[q->f_index]; //pridam c na zaciatok
...
*/
	...
	if(queueEmpty(q) == 1) queueError(QERR_REMOVE); //osetrenie podmienky
	else q->f_index = nextIndex(q->f_index); //posuniem ukazovatel na frontu
	  //solved = FALSE;                  /* V p��pad� �e�en�, sma�te tento ��dek! */
}
...
*/
	...
	if(queueEmpty(q) == 1) queueError(QERR_GET); //osetrenie podmienky
	else{
		queueFront(q, c); //ziskam prvy znak
		queueRemove(q); //posuniem ukazovatel
	}
	  //solved = FALSE;                  /* V p��pad� �e�en�, sma�te tento ��dek! */
}
...
*/
	...
	if (queueFull(q) == 1) queueError(QERR_UP); //osetrenie podmienky
	else{
		q->arr[q->b_index] = c; //na prve volne miesto vlozim prvok
		q->b_index = nextIndex(q->b_index); //posuniem ukzovatel
	}
//	  solved = FALSE;                  /* V p��pad� �e�en�, sma�te tento ��dek! */
}
