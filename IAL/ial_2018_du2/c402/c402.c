...
**/
	...
	while (TRUE) {
		if ((*RootPtr) == NULL){ //ak som na mieste kam treba node pridat
			(*RootPtr) = malloc(sizeof(struct tBTNode));
			if ((*RootPtr) == NULL) break;
		...
		else if ((*RootPtr)->Cont > Content){ //ak je uzol mensi ako otec idem do laveho podstromu
			RootPtr = &(*RootPtr)->LPtr;
			continue;
		}
		else if ((*RootPtr)->Cont < Content){ //ak je uzol vacsi ako otec idem do praveho podstromu
			RootPtr = &(*RootPtr)->RPtr;
			continue;
		}
		else break; //ak uz uzol s hodnotou Content existuje, nerobim nic
	}
...
**/
	...
	tStackP Stack;
	SInitP(&Stack); //inicializujem si stack

	Leftmost_Preorder(RootPtr, &Stack); //zavolam Leftmost_Preorder aby som sa posunul na najviac lavy node

	while (SEmptyP(&Stack) != TRUE){ //pokial nevyprazdnim stack musim pokracovat
		RootPtr = STopPopP(&Stack); //zoberiem si ten najvrchnejsi ukazatel na node
		Leftmost_Preorder(RootPtr->RPtr, &Stack); //a prechadzam vsetky lave podstromy praveho uzlu (vzdy sa posuniem akokeby na pravy node laveho uzlu)
		//workout nevolam, to uz robi funckia Leftmost_Preorder
	}
...
**/
	...
	for (; ptr != NULL; ptr = ptr->LPtr) SPushP(Stack, ptr); //prejdem na najviac lavy node a pointer na kazdy prejdety node ulozim do stacku
...
**/
	...
	//rovnaka funkcia ako BTPostorder
	tStackP Stack;
	SInitP(&Stack);
	...
	while (SEmptyP(&Stack) != TRUE){
		...
		BTWorkOut(RootPtr); //rozdiel je ze node spracujem az tu
...
**/
	...
	for (; ptr != NULL; ptr = ptr->LPtr){ //prechadzam lave podstromy
		SPushP(StackP, ptr); //naplnim zasobnik ukazovatelov
		SPushB(StackB, FALSE); //je prechadzany prvykrat, este sa teda nema spracovat
	}
	 //solved = FALSE;		  /* V případě řešení smažte tento řádek! */
}
...
**/
	...
	if (RootPtr != NULL){
		...
		tStackB BooleanStack;
		SInitB(&BooleanStack); //inicializacia stackov

		Leftmost_Postorder(RootPtr, &PointerStack, &BooleanStack); //prejdem na najviac lavy node

		while (SEmptyP(&PointerStack) != TRUE){ //pokial je nieco v zasobniku tak pokracujem

			RootPtr = STopPopP(&PointerStack); //zoberiem si vrchny pointer

			if (STopPopB(&BooleanStack) == FALSE){ //pokial ma hodnotu FALSE, este ho nechcem spracovat
				SPushB(&BooleanStack, TRUE); //uz som ho ale presiel tak do stacku vratim hodnotu TRUE
				Leftmost_Postorder(RootPtr->RPtr, &PointerStack, &BooleanStack); //a prejdem lave podstromy jeho praveho podstromu
			}
			else BTWorkOut(RootPtr); //ak uz ma hodnotu TRUE, cize uz som ho presiel, mozem ho spracovat
		}
	}
		 //solved = FALSE;		  /* V případě řešení smažte tento řádek! */
}
...
**/
	...
	if (*RootPtr != NULL){ //nechcem mazat prazdny strom
		tStackP Stack;
		SInitP(&Stack); //inicializaia stacku

		tBTNodePtr tmp = *RootPtr;
		SPushP(&Stack, tmp); //root si nahram do stacku

		while (SEmptyP(&Stack) != TRUE){ //idem az kym ho nevyprazdnim
			tmp = STopPopP(&Stack); //vzdy si zoberiem vrch stacku
			if (tmp->LPtr != NULL) SPushP(&Stack, tmp->LPtr); //pokial ma nejake podstromy tak ich tam nahram
			if (tmp->RPtr != NULL) SPushP(&Stack, tmp->RPtr);
			free(tmp); //node uvolnim
		}
		BTInit(RootPtr); //nasledne strom nastavim na NULL
	}
